...
echo "RACING_MIRROR_TOPOLOGY role=$role lane=0 worker=0 vcpu=0 lane_to_worker=0:0 lane_to_cpu=0:0 worker_qd=1 lanes=1 aggregate_outstanding=1 completion=both_terminal_fdatasync"
echo "RACING_MIRROR_TOPOLOGY_WARNING functional_qemu=true representative_benchmark=false hugetlb=absent memlock_headroom=unverified kthread_affinity=unverified hctx_affinity=unverified batching=disabled io_uring_fast_path=unused raw_transport_rtt=not_measured theoretical_iops_ceiling=not_reported"
...
case "$role" in
	remote-leaf)
		...
		truncate_bytes=$(((frames - 1) * (64 + 4096)))
		truncate -s "$truncate_bytes" /mnt/remote.log || fail inject-remote-lag
		sync
		echo "RACING_MIRROR_QEMU_LAG_INJECTED role=$role durable_frames=$((frames - 1))"
	...
	first-hop)
		...
		;;
	client)
		/zcracing-mirror client 10.44.0.2:47030 "$frames" 4096 || fail client
		echo "RACING_MIRROR_QEMU_RESTART role=$role recovered_frames=$frames"
		/zcracing-mirror client 10.44.0.2:47030 "$resume_frames" 4096 || fail client-resume
		;;
esac
