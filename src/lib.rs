...
fn connect_tcp_bound_local_addr(remote: SocketAddr, local: SocketAddr) -> io::Result<TcpStream> {
    let domain = match (remote, local) {
        (SocketAddr::V4(_), SocketAddr::V4(_)) => libc::AF_INET,
        (SocketAddr::V6(_), SocketAddr::V6(_)) => libc::AF_INET6,
        _ => {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "local and remote socket address families must match",
            ));
        }
    };
    let fd = unsafe { libc::socket(domain, libc::SOCK_STREAM | libc::SOCK_CLOEXEC, 0) };
    ...
    let result = (|| -> io::Result<()> {
        match (remote, local) {
            (SocketAddr::V4(remote), SocketAddr::V4(local)) => {
                let local = sockaddr_in_for(local);
                let ret = unsafe {
                    libc::bind(
                        fd,
                        &local as *const libc::sockaddr_in as *const libc::sockaddr,
                ...
                let remote = sockaddr_in_for(remote);
                let ret = unsafe {
                    libc::connect(
                        fd,
                        &remote as *const libc::sockaddr_in as *const libc::sockaddr,
            ...
            (SocketAddr::V6(remote), SocketAddr::V6(local)) => {
                let local = sockaddr_in6_for(local);
                let ret = unsafe {
                    libc::bind(
                        fd,
                        &local as *const libc::sockaddr_in6 as *const libc::sockaddr,
                ...
                let remote = sockaddr_in6_for(remote);
                let ret = unsafe {
                    libc::connect(
                        fd,
                        &remote as *const libc::sockaddr_in6 as *const libc::sockaddr,
...
fn connect_tcp_bound_local_ip(remote: SocketAddr, local_ip: IpAddr) -> io::Result<TcpStream> {
    let local = match (remote, local_ip) {
        (SocketAddr::V4(_), IpAddr::V4(ip)) => SocketAddr::V4(SocketAddrV4::new(ip, 0)),
        (SocketAddr::V6(_), IpAddr::V6(ip)) => {
            SocketAddr::V6(std::net::SocketAddrV6::new(ip, 0, 0, 0))
        }
        _ => {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "local data address family does not match remote data address family",
            ));
        }
    };
...
fn zc_tcpmux_resolve_remote_addr(
    data_host: &str,
    port: u16,
    local_ip: Option<IpAddr>,
) -> io::Result<SocketAddr> {
    let addrs = (data_host, port).to_socket_addrs()?;
    for addr in addrs {
        match (local_ip, addr) {
            (Some(IpAddr::V4(_)), SocketAddr::V4(_))
            | (Some(IpAddr::V6(_)), SocketAddr::V6(_))
            | (None, _) => return Ok(addr),
            _ => {}
        }
    }
    Err(io::Error::new(
        io::ErrorKind::NotFound,
        format!("no remote socket address for {data_host}:{port} matching local bind family"),
    ))
}

fn zc_tcpmux_connect_tcp(
    data_host: &str,
    port: u16,
    local_data_address: Option<&str>,
    context: &str,
) -> io::Result<TcpStream> {
    let local_ip = local_data_address
        .map(|value| {
            value.parse::<IpAddr>().map_err(|err| {
                io::Error::new(
                    io::ErrorKind::InvalidInput,
                    format!("parse --local-data-address {value:?}: {err}"),
                )
            })
        })
        .transpose()?;
    let remote = zc_tcpmux_resolve_remote_addr(data_host, port, local_ip)?;
    let stream = if let Some(local_ip) = local_ip {
        connect_tcp_bound_local_ip(remote, local_ip)
    } else {
    ...
    .map_err(|err| {
        io::Error::new(
            err.kind(),
            format!(
                "{context} {data_host}:{port} local_data_address={}: {err}",
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            other => {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidInput,
                    format!("unknown zcmaptee option {other:?}"),
                ));
            }
        }
    }
    ...
    match (mode.as_deref(), positional.as_slice()) {
        ...
        (Some("push"), [remote]) => {
            let (host, remote_dest) = zc_tcpmux_endpoint(remote).ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::InvalidInput,
                    "zc-tcpmux-xfer push with stdin requires HOST:DEST",
                )
            })?;
            let data_paths = zc_tcpmux_build_data_paths(
                host,
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            other => {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidInput,
                    format!("unknown zcmeter option {other:?}"),
                ));
            }
        }
    }
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            other => {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidInput,
                    format!("unknown zcwritebench option {other:?}"),
                ));
            }
        }
    }
