...
OUTDIR="$ROOT/bench-results/zcworkload-qemu-tap-2lane-dualnic-repeat-20260713T1045Z"
KERNEL=/home/rob/src/linux-7.0.8-zcslots/arch/x86/boot/bzImage
INITRD="$OUTDIR/zcworkload-two-vm-initramfs.cpio"
BRIDGE0=zcwlbr0
BRIDGE1=zcwlbr1
TARGET_TAP0=zcwltgt0
TARGET_TAP1=zcwltgt1
CLIENT_TAP0=zcwlcli0
CLIENT_TAP1=zcwlcli1
...
cleanup()
{
	...
	set +e
	{
		...
		for link in "$TARGET_TAP0" "$TARGET_TAP1" "$CLIENT_TAP0" "$CLIENT_TAP1" "$BRIDGE0" "$BRIDGE1"; do
...
for tap in "$TARGET_TAP0" "$TARGET_TAP1" "$CLIENT_TAP0" "$CLIENT_TAP1"; do
	sudo ip tuntap add dev "$tap" mode tap user "$(id -un)"
done
sudo ip link set "$TARGET_TAP0" master "$BRIDGE0"
sudo ip link set "$CLIENT_TAP0" master "$BRIDGE0"
sudo ip link set "$TARGET_TAP1" master "$BRIDGE1"
sudo ip link set "$CLIENT_TAP1" master "$BRIDGE1"
for link in "$BRIDGE0" "$BRIDGE1" "$TARGET_TAP0" "$TARGET_TAP1" "$CLIENT_TAP0" "$CLIENT_TAP1"; do
	sudo ip link set "$link" mtu 9000 up
done
{
	for link in "$BRIDGE0" "$BRIDGE1" "$TARGET_TAP0" "$TARGET_TAP1" "$CLIENT_TAP0" "$CLIENT_TAP1"; do
...
/usr/bin/time -v -o "$OUTDIR/target-qemu-time.log" \
	...
	qemu-system-x86_64 \
		...
		-netdev tap,id=link0,ifname="$TARGET_TAP0",script=no,downscript=no \
		-device virtio-net-pci,netdev=link0,mac=52:54:00:72:00:02 \
		-netdev tap,id=link1,ifname="$TARGET_TAP1",script=no,downscript=no \
		-device virtio-net-pci,netdev=link1,mac=52:54:00:73:00:02 &
TARGET_JOB_PID=$!
...
/usr/bin/time -v -o "$OUTDIR/client-qemu-time.log" \
	...
	qemu-system-x86_64 \
		...
		-netdev tap,id=link0,ifname="$CLIENT_TAP0",script=no,downscript=no \
		-device virtio-net-pci,netdev=link0,mac=52:54:00:72:00:01 \
		-netdev tap,id=link1,ifname="$CLIENT_TAP1",script=no,downscript=no \
		-device virtio-net-pci,netdev=link1,mac=52:54:00:73:00:01 &
CLIENT_JOB_PID=$!
...
echo "host-harness-result: PASS two_vm_kvm=true tap_bridges=2 virtual_nics_per_vm=2 lanes=2 direct_userspace_memory_leaf=true frame_bytes=4096 representative=false"
