...
OUTDIR="$ROOT/bench-results/zcworkload-qemu-quickack-tail-qd4-20260713T0220Z"
...
test -c /dev/kvm
test -n "$COORD_TOKEN"
if ss -H -ltn "sport = :$LINK_PORT" | grep -q .; then
	echo "host link port $LINK_PORT is already in use" >&2
	exit 10
fi
...
/usr/bin/time -v -o "$OUTDIR/target-qemu-time.log" \
	...
	qemu-system-x86_64 \
		...
		-netdev socket,id=link,listen=127.0.0.1:$LINK_PORT \
...
for _ in $(seq 1 100); do
	ss -H -ltn "sport = :$LINK_PORT" | grep -q . && break
	sleep 0.1
done
ss -H -ltn "sport = :$LINK_PORT" | grep -q .
...
/usr/bin/time -v -o "$OUTDIR/client-qemu-time.log" \
	...
	qemu-system-x86_64 \
		...
		-netdev socket,id=link,connect=127.0.0.1:$LINK_PORT \
...
pin_vm target "$TARGET_PID" 12 4 "$OUTDIR/host-thread-map.log"
pin_vm client "$CLIENT_PID" 13 8 "$OUTDIR/host-thread-map.log"
...
echo "host-harness-result: PASS two_vm_kvm=true direct_userspace_memory_leaf=true frame_bytes=4096 representative=false"
