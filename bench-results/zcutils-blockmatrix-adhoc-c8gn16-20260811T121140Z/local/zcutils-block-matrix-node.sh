...
worker_cpus() {
	...
	1) printf '3\n' ;;
	2) printf '3,35\n' ;;
	*) die "campaign supports one or two workers, got $1" ;;
	esac
}
...
leaf_start() {
	local tag="${1:?tag}" transport="${2:?transport}" mode="${3:?mode}" lanes="${4:?lanes}" qd="${5:?qd}" tree="${6:-current}"
	...
	*) die "unknown mode $mode" ;;
	...
	mkdir -p "$out"
	safe_stop_pidfile "$out/leaf.pid" zcnblk-wal-lea
	...
	printf 'leaf_ready=true tag=%s transport=%s mode=%s lanes=%s qd=%s pid=%s\n' \
		"$tag" "$transport" "$mode" "$lanes" "$qd" "$pid"
}

zcnblk_run() {
	local tag="${1:?tag}" transport="${2:?transport}" mode="${3:?mode}" lanes="${4:?lanes}" qd="${5:?qd}" fua="${6:-0}" tree="${7:-current}"
	local tree_dir read_percent owner_ingress=0 rma_reads=0 rma_writes=0 representative=1 pipeline_batches=16
	...
	*) die "unknown mode $mode" ;;
	...
	env \
		...
		URING_PLAY_ZCNBLK_SHM_OFI_RMA_WRITE_QD="$qd" \
...
nvmet_target_setup() {
	...
	done
	{
		...
		printf 'target_worker_cpus=3,35 irq_cpus=56-63 route_interface=%s\n' "$(route_interface "$CLIENT_IP")"
...
nvmet_client_connect() {
	...
	} >"$RUN_ROOT/nvmet/client/client.env"
	{
		...
		printf 'client_worker_cpus=0,32 irq_cpus=56-63 route_interface=%s route_source=%s\n' \
...
nvme_run() {
	...
	*) die "unknown mode $mode" ;;
	...
	mkdir -p "$out"
	{
		...
		printf 'lane_to_worker_cpu=%s\n' "$(csv_map '' "$cpus")"
		printf 'client_cpu_list=%s target_worker_cpus=3,35 irq_cpus=56-63\n' "$cpus"
