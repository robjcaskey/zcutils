...
OUTDIR="$ROOT/bench-results/zcworkload-qemu-tap-2lane-mq-vhostdedicated-qd64-20260713T0915Z"
...
cleanup()
{
	...
	set +e
	{
		...
		sudo setfacl -x "u:$(id -un)" /dev/vhost-net 2>/dev/null || true
	...
	for _ in $(seq 1 100); do
		...
		for task in /proc/$pid/task/[0-9]*; do
			...
			case "$comm" in
				vhost-*)
					if [ "$found" -eq 0 ]; then
						host_cpu=$first_host_cpu
					else
						host_cpu=$second_host_cpu
					fi
					taskset -pc "$host_cpu" "$tid" >> "$out" 2>&1
					printf 'vhost-map: vm=%s qemu_pid=%s tid=%s queue=%s host_cpu=%s\n' \
						"$label" "$pid" "$tid" "$found" "$host_cpu" >> "$out"
					found=$((found + 1))
					;;
			esac
		done
...
test -r "$INITRD"
test -c /dev/kvm
test -c /dev/vhost-net
test -n "$COORD_TOKEN"

sudo setfacl -m "u:$(id -un):rw" /dev/vhost-net
sudo ip link del "$TARGET_TAP" 2>/dev/null || true
sudo ip link del "$CLIENT_TAP" 2>/dev/null || true
sudo ip link del "$BRIDGE" 2>/dev/null || true
sudo ip link add "$BRIDGE" type bridge
sudo ip tuntap add dev "$TARGET_TAP" mode tap multi_queue user "$(id -un)"
sudo ip tuntap add dev "$CLIENT_TAP" mode tap multi_queue user "$(id -un)"
...
/usr/bin/time -v -o "$OUTDIR/target-qemu-time.log" \
	...
	qemu-system-x86_64 \
		...
		-m 2048M -smp 3,sockets=1,cores=3,threads=1 \
		...
		-netdev tap,id=link,ifname="$TARGET_TAP",script=no,downscript=no,queues=2,vhost=on \
		-device virtio-net-pci,netdev=link,mac=52:54:00:72:00:02,mq=on,vectors=6 &
TARGET_JOB_PID=$!
...
/usr/bin/time -v -o "$OUTDIR/client-qemu-time.log" \
	...
	qemu-system-x86_64 \
		...
		-netdev tap,id=link,ifname="$CLIENT_TAP",script=no,downscript=no,queues=2,vhost=on \
		-device virtio-net-pci,netdev=link,mac=52:54:00:72:00:01,mq=on,vectors=6 &
CLIENT_JOB_PID=$!
...
pin_vm target "$TARGET_PID" 2 4 3 "$OUTDIR/host-thread-map.log"
pin_vm client "$CLIENT_PID" 15 8 7 "$OUTDIR/host-thread-map.log"
pin_vhost_vm target "$TARGET_PID" 16 17 "$OUTDIR/host-thread-map.log"
pin_vhost_vm client "$CLIENT_PID" 19 23 "$OUTDIR/host-thread-map.log"
...
echo "host-harness-result: PASS two_vm_kvm=true tap_bridge=true virtio_queue_pairs=2 vhost_dedicated_physical_cores=true lanes=2 direct_userspace_memory_leaf=true frame_bytes=4096 representative=false"
