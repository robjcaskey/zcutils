...
static bool disable_merges = true;
module_param(disable_merges, bool, 0444);
MODULE_PARM_DESC(disable_merges,
		 "Disable blk-mq merge attempts for latency-sensitive random I/O");
...
MODULE_PARM_DESC(hctx_numa_node,
		 "Distribute every blk-mq hardware queue across CPUs on this NUMA node; -3 maps whole physical cores evenly across all queues; -2 splits queues evenly across NUMA nodes while keeping SMT siblings together; -1 preserves the kernel default map");
...
struct zcnblk_conn {
	...
	/* Producer-owned under queue_lock; workers may observe with READ_ONCE. */
	u32 pending_count;
	...
	u64 tx_seq;
	u64 rx_seq;
	/* Single connection-worker owner; debugfs observes with READ_ONCE. */
	u64 bio_alias_writes;
	u64 bio_alias_reads;
	u8 *rx_plaintext;
...
struct zcnblk_shm_state {
	...
	/* Serialized cold-path ownership of blk-mq dispatch quiescence. */
	struct mutex dispatch_freeze_lock;
	bool dispatch_frozen;
	bool registered;
...
static int zcnblk_shm_claim_payload_slot(struct zcnblk_conn *conn, u32 *slot)
{
	...
	/*
	 * The shared free count is advisory for the winning cmpxchg, but it is an
	 * exact fast rejection when zero.  Without this gate, retained early-ack
	 * writes can exhaust the lease pool and make every lane scan all 16K owner
	 * words in a tight loop while it waits for userspace to release one.
	 */
	if (!atomic64_read((atomic64_t *)&channel->payload_free_slots))
		return -EAGAIN;
...
static bool zcnblk_shm_has_capacity(struct zcnblk_conn *conn)
{
	...
	if (conn->dev->shm->transfer_payload_slots) {
		/*
		 * Ordinary block bios require a kernel-free transferred slot.  Make
		 * lease pressure a real wait condition so the worker arms its wake
		 * and bounded idle recheck instead of spinning through failed owner
		 * scans.  Imported application-arena bios may own an exact reserved
		 * slot and therefore retain their separate alias-aware admission path.
		 */
		if (!shm_bio_arena_zero_copy &&
...
static bool zcnblk_shm_completion_ready(struct zcnblk_conn *conn)
{
	struct zcnblk_shm_channel *channel =
		zcnblk_shm_channel(conn->dev, conn->conn_id);
	u64 consumed = READ_ONCE(channel->comp_cons);
	u64 produced = smp_load_acquire(&channel->comp_prod);
	struct zcnblk_shm_completion *completion;

	if (produced == consumed)
		return false;
	/*
	 * comp_prod is a reservation/publication frontier, not by itself proof
	 * that the completion at comp_cons is consumer-visible.  In particular,
	 * a producer that is descheduled between advancing its frontier and the
	 * head descriptor becoming coherent must not make this predicate stay
	 * true forever: every wait below uses it, and that would turn a transient
	 * publication gap into a non-preemptible kthread spin/RCU stall.
	 *
	 * Match the consume-side acquire check here.  A frontier-ahead but
	 * head-not-ready lane falls through to the bounded poll/wait path and is
	 * retried without lying to wait_event about runnable work.
	 */
	completion = zcnblk_shm_completion(conn->dev, conn->conn_id, consumed);
	return smp_load_acquire(&completion->sequence) == consumed + 1;
}
...
static int zcnblk_debugfs_state_show(struct seq_file *m, void *unused)
{
	...
	u64 bio_alias_writes = 0;
	u64 bio_alias_reads = 0;
	u32 conn_id;
	...
	for (conn_id = 0; conn_id < dev->total_conns; conn_id++) {
		bio_alias_writes += READ_ONCE(dev->conns[conn_id].bio_alias_writes);
		bio_alias_reads += READ_ONCE(dev->conns[conn_id].bio_alias_reads);
	}
	seq_printf(m,
		   "shm=1 daemon_online=%llu daemon_generation=%llu global_submit_sequence=%llu published_submit_sequence=%llu sequence_telemetry_interval=%u completion_batch=%u ring_entries=%u payload_entries=%u transfer_payload_slots=%u arena_backing=%s region_bytes=%zu bio_arena_zero_copy=%u bio_arena_zero_copy_required=%u bio_alias_writes=%llu bio_alias_reads=%llu bio_alias_busy_fallbacks=%lld bio_alias_required_retries=%lld bio_alias_required_rejects=%lld\n",
				 ...
				 struct zcnblk_pdu *pdu)
{
	...
	conn->inflight_count++;
	/*
	 * The target keeps this flag clear while it is polling a busy lane.  Avoid
	 * issuing a locked xchg for every request in that steady state; only race
	 * to disarm and wake after observing that the target may be asleep.  The
	 * second check remains atomic, so a concurrent producer still emits at
	 * most one wake for each armed interval.
	 */
	if (READ_ONCE(channel->request_wake_armed) &&
	    xchg(&channel->request_wake_armed, 0)) {
...
static int zcnblk_shm_conn_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		/*
		 * Consume one bounded completion batch before returning to the
		 * submit side.  Draining until comp_prod is caught can be an
		 * unbounded loop when userspace produces completions continuously;
		 * on a non-preemptible kernel that starves RCU and eventually trips
		 * the hard/soft-lockup watchdog.  The outer loop preserves the same
		 * lane-local hot path while alternating admission and retirement.
		 */
		ret = zcnblk_shm_consume_completions(conn);
		if (ret > 0)
			progressed = true;
		if (ret < 0) {
			conn->failed = true;
		...
		if (conn->failed)
			break;
		/*
		 * Both a saturated lane and a transient producer/list observation race
		 * can bypass the wait path indefinitely.  Account every outer-loop
		 * quantum, including a no-progress quantum whose following predicate
		 * says work is ready.  Otherwise resetting this counter on no-progress
		 * can create a non-preemptible busy loop and starve RCU forever.
		 */
		loop_quanta++;
		if (unlikely(!(loop_quanta & 63)))
			cond_resched();
		if (unlikely(loop_quanta == 16384)) {
				/*
				 * cond_resched() is deliberately a no-op on some fully
				 * preemptible kernels when this dedicated CPU has no other
				 * runnable task.  That still leaves the watchdog and RCU
				 * without an observed context switch.  Force one very short
				 * sleep after a large, bounded amount of sustained work.  At
				 * 256 completions per quantum this is one sleep per 4M I/Os;
				 * it is outside descriptor admission and cannot contend with
				 * another lane.
				 */
				usleep_range(1, 2);
				loop_quanta = 0;
...
static void zcnblk_wake_conn_for_submission(struct zcnblk_conn *conn)
{
	struct zcnblk_shm_channel *channel;
	...
	channel = zcnblk_shm_channel(conn->dev, conn->conn_id);
	/*
	 * The shared completion worker arms this flag immediately before its
	 * condition recheck and sleep.  Avoid taking the waitqueue lock for every
	 * queue_rq() while that worker is already running.  Either this producer
	 * or the userspace completion producer wins the disarm; the winner wakes
	 * the same worker, and the worker's post-arm condition check closes the
	 * store-before-arm race.
	 */
	if (smp_load_acquire(&channel->completion_wake_armed) &&
	    xchg(&channel->completion_wake_armed, 0))
		wake_up(&conn->wait);
}
				    ...
				    const struct blk_mq_queue_data *bd)
{
	...
	zcnblk_wake_conn_for_submission(conn);
	return BLK_STS_OK;
}
...
static void zcnblk_map_queues(struct blk_mq_tag_set *set)
{
	...
	if (hctx_numa_node == -3) {
		...
		/*
		 * blk_mq_map_queues() distributes logical CPUs.  At queue counts
		 * near the physical-core count that can put SMT siblings in
		 * different hctxs and leave no collision-free three-role map for
		 * the client, userspace target, and completion kthread.  This is a
		 * topology-time map only: keep each sibling set together and divide
		 * physical cores evenly, with no request-path policy or placement.
		 */
		...
		for_each_possible_cpu(cpu) {
			unsigned int core_rank = 0;
			unsigned int peer;
			unsigned int queue;
			bool found = false;

			for_each_possible_cpu(peer) {
				if (cpumask_first(topology_sibling_cpumask(peer)) != peer)
					continue;
				if (cpumask_test_cpu(cpu,
						     topology_sibling_cpumask(peer))) {
					found = true;
					break;
				}
				core_rank++;
			...
			if (!found)
				core_rank = cpu % total_cores;
			queue = div_u64((u64)core_rank * qmap->nr_queues,
					 total_cores);
	...
	if (hctx_numa_node == -2) {
		...
		for_each_possible_cpu(cpu) {
			...
			if (queue_end <= queue_begin) {
				...
				queue = queue_begin +
					div_u64((u64)core_rank *
						(queue_end - queue_begin), node_cores);
				if (queue >= queue_end)
					queue = queue_end - 1;
			}
			qmap->mq_map[cpu] = qmap->queue_offset + queue;
	...
	for_each_possible_cpu(cpu) {
		...
		if (cpu_to_node(cpu) == hctx_numa_node) {
			...
			unsigned int remote_cpus = num_possible_cpus() - local_cpus;

			/*
			 * blk-mq requires a map for every possible CPU.  These CPUs are
			 * outside the explicitly selected NUMA-local benchmark topology,
			 * but assigning all of them to queue zero turns hctx0 into a
			 * system-wide fallback choke point.  Spread that fallback evenly;
			 * placement remains a topology-time property and userspace still
			 * owns all storage/lane placement decisions.
			 */
			queue = remote_cpus ?
				div_u64((u64)remote_index * qmap->nr_queues,
					  remote_cpus) : 0;
			if (queue >= qmap->nr_queues)
				queue = qmap->nr_queues - 1;
			remote_index++;
...
static int zcnblk_shm_ctl_release(struct inode *inode, struct file *file)
{
	...
	if (shm->dispatch_frozen && zcnblk_dev && zcnblk_dev->disk) {
		blk_mq_unquiesce_queue(zcnblk_dev->disk->queue);
				 ...
				 unsigned long arg)
{
	...
	case ZCNBLK_SHM_IOC_KICK:
		...
		return 0;
	case ZCNBLK_SHM_IOC_FREEZE_DISPATCH:
		if (!zcnblk_dev || !zcnblk_dev->disk)
			return -ENODEV;
		mutex_lock(&shm->dispatch_freeze_lock);
		...
		/*
		 * Stop new dispatch first, then use an expedited grace period for
		 * the dispatch-side RCU/SRCU domain.  This has the same exact
		 * queue_rq admission fence as blk_mq_quiesce_queue(), but avoids a
		 * normal multi-jiffy grace period during the rare migration cutover.
		 * It adds no branch, counter, or cache-line traffic to queue_rq().
		 */
		blk_mq_quiesce_queue_nowait(zcnblk_dev->disk->queue);
		if (zcnblk_dev->tag_set.flags & BLK_MQ_F_BLOCKING)
			synchronize_srcu_expedited(zcnblk_dev->tag_set.srcu);
		else
			synchronize_rcu_expedited();
		shm->dispatch_frozen = true;
		mutex_unlock(&shm->dispatch_freeze_lock);
		wake_up_interruptible_poll(&shm->poll_wait, EPOLLIN | EPOLLRDNORM);
		return 0;
	case ZCNBLK_SHM_IOC_THAW_DISPATCH:
		if (!zcnblk_dev || !zcnblk_dev->disk)
			return -ENODEV;
		mutex_lock(&shm->dispatch_freeze_lock);
		if (!shm->dispatch_frozen) {
...
static int zcnblk_shm_layout_init(struct zcnblk_dev *dev)
{
	...
	mutex_init(&shm->dispatch_freeze_lock);
	xa_init(&shm->arena_page_indices);
...
static int __init zcnblk_init(void)
{
	...
	if (disable_merges)
		blk_queue_flag_set(QUEUE_FLAG_NOMERGES, zcnblk_dev->disk->queue);
