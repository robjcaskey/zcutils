...
/*
 * Cold-path migration fence. FREEZE waits for all queue_rq callbacks which
 * were already dispatching, so userspace can drain a finite published-ring
 * vector before changing its downstream route. No placement policy crosses
 * this ABI and queue_rq contains no migration branch.
 */
